		...
		<div id="list-escuela" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="password" title="${message(code: 'escuela.password.label', default: 'Password')}" />
				...
				<g:each in="${escuelaInstanceList}" status="i" var="escuelaInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${escuelaInstance.id}">${fieldValue(bean: escuelaInstance, field: "usuario")}</g:link></td>
					
						<td>${fieldValue(bean: escuelaInstance, field: "password")}</td>
